...
dim(net$embedding$weight)
ct<-matrix(net$embedding$weight)
names(net)
str(net)
#net$
query<-"unten"
...
nietzsche.c<-unlist(strsplit(corpus,""))
lc<-length(unique(nietzsche.c)) #chk 89
nietzsche_matrix<-matrix(nietzsche.c,vocab_size,EMBED_SIZE)
ctm<-net$embedding$weight

library(proxy)
??proxy

install.packages("proxy")
query_emb<-unlist(strsplit(query,""))
chars <- c("p","h","y","s","i","o","l","o","g","i","e")
vocab<-unique(nietzsche.c)
vocab
length(vocab)
ids <- vocab[chars]
ids <- vocab[query_emb]
ids <- query_emb%in%chars
ids <-vocab%in%query_emb
ids
emb <- net$embedding$weight

vecs <- emb[ids,]
vecs
query_vec <- torch_mean(vecs, dim=1)
query_vec
sim <- proxy::sim(
    query_emb,
    nietzsche_matrix,
