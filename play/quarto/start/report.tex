...
\newcommand{\doctitle}{PDF Report with Wrapped Code}
...
\urlstyle{same}
\hypersetup{
  pdftitle={PDF Report with Wrapped Code},
  colorlinks=true,
  linkcolor={blue},
  filecolor={Maroon},
  citecolor={Blue},
...
\title{PDF Report with Wrapped Code}
\author{}
\date{}
\begin{document}
\maketitle
...
\section{PDF report}\label{pdf-report}

This document will be rendered to \texttt{report.pdf}. Long code lines
will wrap in the PDF.
...
\CommentTok{\#tx\textless{}{-}readtext::readtext(t)$text}
\CommentTok{\#tx}
\DocumentationTok{\#\#}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{cat}\NormalTok{(}\StringTok{"\textasciigrave{}\textasciigrave{}\textasciigrave{}text}\SpecialCharTok{\textbackslash{}n}\StringTok{"}\NormalTok{,}
    \FunctionTok{paste}\NormalTok{(txt, }\AttributeTok{collapse =} \StringTok{"}\SpecialCharTok{\textbackslash{}n}\StringTok{"}\NormalTok{),}
    \StringTok{"}\SpecialCharTok{\textbackslash{}n}\StringTok{\textasciigrave{}\textasciigrave{}\textasciigrave{}"}\NormalTok{,}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{System prompt: You are a member of the german parliament. You will be provided a plenary protocol text. Please summarize the text for presentation to your local community. Output summary in german language. Constraints: No preamble, output summary as plaintext with no extra formatting, limit summary to 20\% of input text.}

\NormalTok{Plenary protocol text: \_bttx\_}
\end{Highlighting}
\end{Shaded}
