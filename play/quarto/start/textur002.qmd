...
das zeigt sich dann in zunehmenden musterwiederholungen, eindimensionalen phrasen, weniger types (unique words/tokens) und generell einer abnahme der entropie dh. die annahme eines höher organisierten (aber nicht mehr mensch-sprachlich strukturierten) zustands, der konsequent auf erstarrung hinausläuft, weil die späteren generationen eine weitgehend (inzestuöse beziehung) mit dem eigenen output eingehen. in texturbegriffe übersetzt würde unser teppich langsam den weg von einem farbenfrohen (universum der begriffe) hin zu einem diachromen gewebe aus eigentlich nur 2 zuständen, nämlich strom/nichtstrom, nehmen.

> see: [model-collaps](model-collapse-textur.qmd) for a nice experiment on modeling with a small corpus.

[^1]: ich musz zugeben, ich kommen mit den beiden seminaren literatur/KI und textur etwas durchenander, es lohnt sich aber auch (von hier) einmal [dort](lki-index.qmd) zu schauen, was sich so überschneidet...
