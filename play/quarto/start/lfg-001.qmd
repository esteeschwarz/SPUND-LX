...
c(t="shot",X.N=1,X.A=0,X.D=0,X.V=1,PRED.subj=1,PRED.obj=1,NUM.sg=1,NUM.pl=0,TENSE.pre=0,TENSE.past=1,PERS.1=0,PERS.2=0,PERS.3=0,DEF.plus=0,DEF.minus=0),
...
but: when 'shot' takes (^PRED)="<shoot>" it - coded alone - does not show above ambiguity, saying the predicate here is important for the rest of the row. so further: we either have to somehow embed the predicate reference into the binary system (which is not possible since its again an (infinite space of tokens/ lexical items)) or distinguish tokens by token qualities like 'shot.shoot' vs. 'shot.shot'. 