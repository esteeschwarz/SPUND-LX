...
#### disclaimer
the above displayed binary dataframe is *not* derived from script but manually created as its only visualing what woud be the goal to achieve then via a script, such that a parser (script) would look at a sentence/token and construct the binary code without context available. this is still the main question what can be derived automatically and what not.
