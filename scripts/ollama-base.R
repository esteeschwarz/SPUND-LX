...
llama.quest<-function(model="llama3.2",sysprompt.file=NULL,text=NULL){
...
# prompt
sprompt<-ifelse(!is.null(sysprompt.file),readLines(sysprompt.file),"# system prompt:")
###############################
get.trans<-function(tr){
#  print(tr)
  m<-sprompt=="_text_"
  ifelse(m,p.text<-gsub("_text_",tr,prompt),p.text<-c(sprompt,tr))
  print(p.text)
