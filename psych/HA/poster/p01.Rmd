...
dataset<-3
eval.n<-dataset
ifelse(dataset%in%c(3,5),det.restrict<-'(only in conditions b-d these must also be tagged "DET")',
       det.restrict<-"")
```
# xTitle
...
The dataframe used for modeling consists of ``` `r length(dfa$q)` ``` distance datapoints (sample below) derived from the postagged corpus.

```{r sourcescr,results=F,warning=F,message=F,echo=F,eval=T}
# library(jsonlite)
# library(abind)
...
```{r lmeplot, results=F,message=F,warning=F,echo=FALSE,eval=T}
# source(paste0(Sys.getenv("GIT_TOP"),"/SPUND-LX/psych/HA/eval-002.R"))
#print(head(df))
...
## conclusion
Over all conditions <!--**B** (``` `r unlist(qjs$b)` ```)-->we find significantly higher distance scores in the target corpus which proves our hypothesis. An ANOVA analysis of the linear regression model [cf. @bates_fitting_2015] which posited a main effect of corpus\*q+range and random effects of lemma (`lme4::lmer(dist ~ corp*q+range + (1|lemma`) gets a p-value of ```p=`r p.target` ``` for the mean difference of ``` `r dist.target` ``` tokens (targetref) compared to the target.   
