...
## daten
We built a corpus of the reddit r/schizophrenia thread (```n=`r n_obs` ``` tokens) and a reference corpus of r/unpopularopinion (```n=`r n_ref` ```). The corpus has been pos-tagged using the R udpipe:: package [@wijffels_udpipe_2023] which tags according to the universal dependencies tagset maintained by [@de_marneffe_universal_2021]. Still the `r n_obs` tokens can only, with the workflow of growing the corpus and devising the noun distances developed be just a starting point from where with more datapoints statistical evaluation becomes relevant first.   
