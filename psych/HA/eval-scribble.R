...
lm1<-lmer(dist~target*q+(1|prepos)+(1|det),qltdf)
lm1<-lmer(dist_rel_obs~target*q+(1|prepos)+(1|det),dfa)
lm2<-lmer(dist_rel_obs~target*q+(1|prepos)+(1|det)+(1|aut_id),dfa)
sum1<-summary(lm1)
sum1
sum2<-summary(lm2)
sum2
barplot(sum2$coefficients[,1])
coef<-sum2$coefficients[,1]
coef[1]<-0
par(las=2)
barplot(coef)
anova(lm2)
