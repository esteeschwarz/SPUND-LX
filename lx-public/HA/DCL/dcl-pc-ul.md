...
wie wir sehen, ist "markdown" hier das mittel der wahl, um ziemlich convenient content zu kreiren. mit diesem "textformat" sollte man sich deshalb zumindest auseinandersetzen - was m.e. keine gröszere schwierigkeit darstellt und (auch m.e.) ohnehin der fixen formatierung innerhalb proprietärer oder auch open source WYSIWYG (what-you-see-is-what-you-get) textverarbeitungen [word, pages, openoffice udgl.] vorgezogen werden sollte, da auf diese weise dokumente in freien und mit ausgiebigen features versehenen texteditoren (zb.: [VSCode](https://code.visualstudio.com/) mit git integration) erstellt/geschrieben werden und auf simple weise konsistent formatiert werden können, ohne mit einem haufen von untereinander nicht-kompatiblen stilen und vorlagen kämpfen zu müssen. mit diesem system sollte man sich for convenience sake jdfs. wirklich vertraut machen. markdown. simple is that... / und wer durchaus weiter in seinem word schreiben will, kann das leider trotzdem tun, den text als plain text exportieren und wenn dieser nur minimal "formatiert" (ausgezeichnet=mit markdown versehen) wird, sieht er am ende schon gut aus.

##### markdown auszeichnungssbeispiel
